...
export default function Sidebar({ mobileOpen, onCloseMobile, collapsed }: SidebarProps) {
  ...
  const { productBookingUnreadCount } = useNotifications();
              ...
              >
                ...
                {!collapsed && (
                  <>
                    ...
                    {label === "Purchased Products" && productBookingUnreadCount > 0 && (
                      <span className="ml-auto flex h-5 min-w-[20px] items-center justify-center rounded-full bg-red-500 px-1.5 text-[10px] font-bold text-white">
                        {productBookingUnreadCount > 99 ? "99+" : productBookingUnreadCount}
                      </span>
                    )}
                  </>
                )}
