import { productService, authService, customerService, orderService, cartService, offerService } from './services.js';
...
function stepProduct() {
  ...
  <div class="flex gap-4 mb-5">
    ...
    <div>
      ...
      <div class="text-sm text-fc-slate/60 mt-1">${priceBlock(p, { size: 'text-fc-slate/80 font-medium' })}</div>${(() => { const o=offerService.forProduct(p.name); return o ? `<div class="mt-2 rounded-lg bg-fc-greenlight/60 border border-fc-green/20 px-3 py-2"><div class="text-xs font-semibold text-fc-green">SPECIAL OFFER — ${o.discount}</div><div class="text-[11px] text-fc-slate/60 mt-0.5">${o.title}${o.code ? ` · Code: ${o.code}` : ''}</div></div>` : ''; })()}
...
function validateStep() {
  if (state.step === 1) {
    ...
    if (!name.trim() || !mobile.trim() || (email.trim() && !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email.trim()))) {
      toast('Please enter your name and valid mobile number. Email is optional.', { type: 'error' });
      return false;
    }
  }
  if (state.step === 2) {
