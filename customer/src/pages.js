      ...
      <div class="lg:col-span-2 space-y-4">
        <div class="bg-fc-charcoal text-white rounded-2xl p-6">
          ...
          <div class="space-y-3 text-sm text-white/75">
            <div class="flex items-start gap-2.5"><i data-lucide="user-round" class="w-4 h-4 mt-0.5 shrink-0"></i><span><strong class="text-white">${COMPANY.adminName}</strong><br><span class="text-white/60">Administrator</span></span></div>
            <div class="flex items-start gap-2.5"><i data-lucide="mail" class="w-4 h-4 mt-0.5 shrink-0"></i><a class="hover:text-fc-wheat" href="mailto:${COMPANY.adminEmail}">${COMPANY.adminEmail}</a></div>
            <div class="flex items-start gap-2.5"><i data-lucide="phone" class="w-4 h-4 mt-0.5 shrink-0"></i><span>${[COMPANY.adminMobile, ...(COMPANY.adminMobileNumbers || [])].filter(Boolean).join(' / ')}</span></div>
            <div class="flex items-start gap-2.5"><i data-lucide="building-2" class="w-4 h-4 mt-0.5 shrink-0"></i><span>${COMPANY.name}</span></div>
            <div class="flex items-start gap-2.5"><i data-lucide="mail" class="w-4 h-4 mt-0.5 shrink-0"></i><a class="hover:text-fc-wheat" href="mailto:${COMPANY.email}">${COMPANY.email}</a></div>
            <div class="flex items-start gap-2.5"><i data-lucide="phone" class="w-4 h-4 mt-0.5 shrink-0"></i><span>${[COMPANY.phone, ...(COMPANY.mobileNumbers || [])].filter(Boolean).join(' / ')}</span></div>
            <div class="flex items-start gap-2.5"><i data-lucide="map-pin" class="w-4 h-4 mt-0.5 shrink-0"></i>${COMPANY.address}</div>
          </div>
          <p class="text-xs text-white/40 mt-4 pt-4 border-t border-white/10">${COMPANY.addressNote} · GSTIN ${COMPANY.gstin}</p>
        </div>
