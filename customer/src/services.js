...
function normalizeProduct(p) {
  ...
  // Preserve the original list price before applying the effective selling price.
  const listPrice =
...
export const productService = {
  ...
  bySlug(slug) {
    return this.cache.find((p) => p.slug === slug) || null;
  },
};

/* IMPORTANT:
   Keep this companyService declaration.
   There must NOT be another export const companyService later in this file.
*/
    ...
    this.loadingPromise = (async () => {
      try {
        ...
        Object.assign(COMPANY, {
          ...
          phone: data.mobile ?? COMPANY.phone,
          whatsapp: data.whatsapp ?? COMPANY.whatsapp,
          address: data.address ?? COMPANY.address,
          mobileNumbers: data.mobile_numbers ?? COMPANY.mobileNumbers,
          adminName: data.admin_name ?? COMPANY.adminName,
          adminEmail: data.admin_email ?? COMPANY.adminEmail,
          adminMobile: data.admin_mobile ?? COMPANY.adminMobile,
          adminMobileNumbers: data.admin_mobile_numbers ?? COMPANY.adminMobileNumbers,
          addressNote: data.address_note ?? COMPANY.addressNote,
        ...
        // Keep existing static presentation values as fallback.
      } finally {
        this.loadingPromise = null;
      }
  ...
  async create(payload) {
    const o = await api('/orders', {
      ...
      body: JSON.stringify({
        ...
        order_method: payload.orderMethod || 'delivery',
      }),
    });
...
const LOGO_FALLBACK_SVG =
  ...
  encodeURIComponent(`
    <svg xmlns="http://www.w3.org/2000/svg" width="64" height="64" viewBox="0 0 64 64">
      ...
      <path d="M32 20 V44 M32 44 L24 36 M32 38 L40 30"
...
// Fetches the bundled logo and inlines it as a base64 data URL.
...
/* FIX:
   This is invoiceService, NOT companyService.
   This prevents the duplicated companyService export error.
*/

export const invoiceService = {
  async buildInvoiceHtml(order) {
    const logoSrc = await getLogoDataUrl();
    const company = companyService.get?.() || COMPANY;
    ...
    const unitPriceLabel = `₹${unitPrice.toLocaleString(
      'en-IN',
      {
...
export const offerService = {
  ...
  async load({ force = false } = {}) {
    if (this.cache.length && !force) {
      return this.cache;
    }

    try {
      this.cache = await api('/offers', {
        headers: {},
      });
    } catch {
      this.cache = [];
