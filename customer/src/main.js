...
import { authService, orderService, customerService, productService, cartService, companyService, offerService } from './services.js';
...
async function render({ resetScroll = true } = {}) {
  ...
  if (path === '/' || path === '/shop' || path.startsWith('/product/') || path === '/cart' || path === '/orders' || path.startsWith('/order/') || path.startsWith('/success/')) { try { await orderService.load(); } catch (e) { console.error('Order API unavailable', e); } }
  if (resetScroll) {
  ...
  app.querySelectorAll('[data-cart-add]').forEach(btn => {
    btn.addEventListener('click', async (e) => {
      ...
      if (product && product.price == null) {
        toast('This product is currently available for enquiry. Please contact the company for pricing.', { type: 'error' });
        return;
      }
